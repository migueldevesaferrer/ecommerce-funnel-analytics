...
SELECT SUM(confirmation)::float / NULLIF(SUM(home), 0) AS conversion_rate
FROM session_steps;
...
-- 8. Views para análisis dimensional
...
SELECT
    ...
    SUM(CASE WHEN page_type = 'confirmation' THEN 1 ELSE 0 END)::float / NULLIF(SUM(CASE WHEN page_type = 'home' THEN 1 ELSE 0 END), 0) AS conversion_rate
FROM vw_dimensional_funnel
GROUP BY device_type
ORDER BY conversion_rate DESC;
